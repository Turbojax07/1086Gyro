...
public class GyroIOBNO085 implements GyroIO {
    ...
    // These would normally be put into a constants file, but since these
    // are only used by this class, they are stored in here.
    ...
    public void updateInputs(GyroIOInputs inputs) {
        ...
        inputs.yaw = buffer_16[0] * DEGREE_SCALE * Math.PI / 180 - offset;
        inputs.pitch = buffer_16[1] * DEGREE_SCALE * Math.PI / 180;
        inputs.roll = buffer_16[2] * DEGREE_SCALE * Math.PI / 180;

        inputs.x_vel = (inputs.pitch - previousInputs.pitch) / 0.02;
        inputs.y_vel = (inputs.roll - previousInputs.roll) / 0.02;
        inputs.z_vel = (inputs.yaw - previousInputs.yaw) / 0.02;

        inputs.x_acc = buffer_16[3] * MILLI_G_TO_MS2;
        inputs.y_acc = buffer_16[4] * MILLI_G_TO_MS2;
        inputs.z_acc = buffer_16[5] * MILLI_G_TO_MS2;
    ...
    public void resetGyro() {
        offset -= previousInputs.yaw;
    }

    @Override
    public void resetGyro(double angle) {
        offset -= previousInputs.yaw - (angle * Math.PI / 180);
    }    
}